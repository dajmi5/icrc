...
class PRTMailMessageDraft(models.Model):
    ...
    def _get_subtypes(self):
        ...
        return [
            (
                ...
                "in",
                [
                    ir_model_data_obj._xmlid_to_res_id("mail.mt_comment"),
                    ir_model_data_obj._xmlid_to_res_id("mail.mt_note"),
                ],
            )
        ]
    ...
    def _prepare_model_dict(self, ir_models):
        ...
        for model in ir_models:
            has_name = fields_obj.search_count(
                [("model_id", "=", model.id), ("name", "=", "name")]
            )
            model_dict.update({model.model: [model.name, has_name]})
        return model_dict

    def _prepare_subject_display(
        self, model_dict, default_subject="=== No Reference ==="
    ):
        """
        ...
        result = model_dict.get(self.model)
        if not (result and len(result) == 2):
            return True, default_subject
        subject_display, has_name = result
        if has_name:
            return True, f"{subject_display}: {self.record_ref.name}"
        if self.subject:
            return True, f"{subject_display} => {self.subject}"
    ...
    def _compose_subject(self, model_dict):
        ...
        """
        for rec in self:
            rec.subject_display = rec._prepare_subject_display(model_dict)[1]
    ...
    def _referenceable_models(self):
        ...
        return [
            ...
            for model in ir_model_obj.search(
                [("model", "!=", "mail.channel"), ("transient", "=", False)]
            )
        ]
